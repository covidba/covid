...
def gerarTendenciaEstado(df, populacao):  
  ...
  futherCases = pd.DataFrame.from_dict({"date":pd.date_range(start = df['date'].max()+timedelta(days=1) , end = df['date'].max()+timedelta(days=180))})
...
df1 = pd.read_csv(url, usecols=['date', 'country', 'state', 'city', 'ibgeID', 'newDeaths', 'deaths', 'newCases', 'totalCases', 'deaths_per_100k_inhabitants', 'totalCases_per_100k_inhabitants', 'deaths_by_totalCases', '_source']) #, parse_dates=['data'])
...
#!cd gdrive/My\ Drive && rm -r covid
#!cd gdrive/My\ Drive && git clone https://github.com/covidba/covid

#for estado in lista_estados:
#  dir = '/content/gdrive/My Drive/covid/tendencia/%s/' % estado
#  if not os.path.isdir(dir):
#    !cd gdrive/My\ Drive/covid/tendencia && mkdir $estado

#!cd gdrive/My\ Drive/covid && git add tendencia
#!cd gdrive/My\ Drive/covid && git commit -m "update tendencia"
#!cd gdrive/My\ Drive/covid && git push -f origin master

ja_feitos = ["AC", "AL", "AP", "AM", "CE"]
